...
import { HardDrive, RefreshCcw, CheckCircle2, XCircle, AlertCircle, ExternalLink, Loader2, Clock } from 'lucide-react';
...
export const AdminDriveStatus: React.FC = () => {
  ...
  const { isSyncing, triggerGlobalSync, lastSyncedAt } = useDriveSync();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200 shadow-sm mb-6 flex flex-col md:flex-row gap-8 items-center justify-between">
        <div className="space-y-2 flex-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand/10 text-brand rounded-full mb-2">
            <Clock className="w-3.5 h-3.5" />
            <span className="text-[10px] font-black uppercase tracking-widest">Automated Cloud Sync</span>
          </div>
          <h3 className="text-xl font-black text-slate-900 italic tracking-tight">Drive Sync Interval</h3>
          <p className="text-sm font-medium text-slate-500">
            Control how often the application automatically synchronizes files and records with Google Drive.
          </p>
          {lastSyncedAt && (
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button
            ...
            disabled={isSyncing}
            className="p-3 bg-brand text-white rounded-2xl shadow-xl shadow-brand/20 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
            title="Trigger Manual Global Sync"
          >
