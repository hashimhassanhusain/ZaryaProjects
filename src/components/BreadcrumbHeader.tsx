...
export const BreadcrumbHeader: React.FC<BreadcrumbHeaderProps> = ({ 
  ...
  return (
    <div className={cn("bg-white border-b border-slate-100 px-6 py-4 flex flex-col gap-2 shadow-sm relative z-20", className)}>
        ...
        <nav className={cn("flex items-center gap-1.5 text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none", isRtl && "flex-row-reverse")}>
          <Link to="/" className="hover:text-blue-600 transition-colors">HQ</Link>
          ...
          <span className="text-blue-600">{stripNumericPrefix(displayTitle)}</span>
        </nav>

        <div className="flex items-center justify-between">
          <div className={cn("flex items-center gap-3", isRtl && "flex-row-reverse")}>
            <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tighter italic uppercase drop-shadow-sm flex items-center gap-2">
