...
export const BreadcrumbHeader: React.FC<BreadcrumbHeaderProps> = ({ 
  ...
  return (
    <div className={cn("bg-white dark:bg-surface border-b border-slate-100 dark:border-white/5 px-6 py-4 flex flex-col gap-2 shadow-sm relative z-20", className)}>
        ...
        <nav className={cn("flex items-center gap-1.5 text-[9px] font-black text-text-secondary uppercase tracking-widest leading-none", isRtl && "flex-row-reverse")}>
        ...
        <div className="flex items-center justify-between">
          <div className={cn("flex items-center gap-3", isRtl && "flex-row-reverse")}>
            <h1 className="text-xl md:text-2xl font-black text-text-primary dark:text-white tracking-tighter italic uppercase drop-shadow-sm flex items-center gap-x-2">
              {parentPage && (
                <span className="text-text-secondary/40 font-black inline-flex items-center gap-2">
                   {stripNumericPrefix(parentTitle)}
              ...
              {activeTabLabel && activeTabLabel !== stripNumericPrefix(displayTitle) && (
                <span className="text-text-secondary font-black ml-3 not-italic normal-case text-lg tracking-normal">
                  / {activeTabLabel}
                </span>
              )}
            </h1>
