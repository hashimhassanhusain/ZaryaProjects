      ...
      <div className="space-y-8 pb-20">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900 tracking-tight italic uppercase">Utilization Tracker</h2>
          ...
          <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm">
             {['All', 'Labor', 'Material', 'Equipment'].map((t) => (
               <button
                 ...
                 className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === t ? 'bg-slate-900 text-white' : 'text-slate-400 hover:text-slate-600'}`}
               >
                 {t}
               </button>
             ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic">Top 8 Resource Load</h3>
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white space-y-8 shadow-xl shadow-emerald-900/10">
              <div>
                 <h4 className="text-xl font-black italic tracking-tighter mb-2">Inventory Alerts</h4>
              ...
              <div className="space-y-4">
                 {resources.filter(r => (r.utilized / r.capacity) > 0.95).map(r => (
                   <div key={r.id} className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-2xl">
                      ...
                      <div className="flex-1">
                         <p className="text-xs font-black italic">{r.name}</p>
                 ...
                 {resources.filter(r => (r.utilized / r.capacity) > 0.95).length === 0 && (
                   <p className="text-[10px] font-black uppercase text-center py-10 opacity-30 tracking-widest">No Alerts</p>
                 )}
              </div>

              <button className="w-full py-4 bg-emerald-500 text-slate-900 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2">
        ...
        <div className="bg-white border border-slate-100 rounded-[2.5rem] shadow-sm overflow-hidden">
           <div className="p-8 border-b border-slate-50 flex items-center justify-between">
              <h3 className="text-lg font-black text-slate-900 italic">Resource Master Pool</h3>
              <div className="relative group">
                 <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                 <input placeholder="Search pool..." className="pl-9 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest w-48" />
              </div>
           </div>
           <table className="w-full text-left">
              <thead>
                 <tr className="bg-slate-50/50">
                    <th className="px-8 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Resource Identity</th>
                    <th className="px-8 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Category</th>
                    <th className="px-8 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Rate</th>
                    <th className="px-8 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Status</th>
                    <th className="px-8 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Load</th>
                   ...
                   <tr key={r.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-8 py-6">
                         <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-black text-slate-900 italic">{r.name}</p>
                         </div>
                      </td>
                      <td className="px-8 py-6 uppercase text-[9px] font-black text-slate-400 tracking-widest">{r.type}</td>
                      ...
                      <td className="px-8 py-6">
                         <button className="opacity-0 group-hover:opacity-100 text-[9px] font-black text-emerald-600 uppercase tracking-widest transition-all">Details</button>
                      </td>
                   </tr>
                 ))}
              </tbody>
