...
type MainTab = 'charter' | 'policies' | 'plans' | 'logs';
type PlanSubTab = 'pmp' | 'cmp' | 'qmp' | 'comm' | 'smp' | 'rmp' | 'scope' | 'hrmp' | 'schedule' | 'cost' | 'procurement';
...
export const GovernanceHubView: React.FC<GovernanceHubViewProps> = ({ page }) => {
  ...
  useEffect(() => {
    ...
    if (['1.2.1', '2.1.5', '3.1.1', '3.2.1'].includes(page.id)) setActiveTab('logs');
    if (page.id.startsWith('2.1')) {
      setActiveTab('plans');
        ...
        <main className={cn("flex-1", (activeTab !== 'plans' && activeTab !== 'logs') && "w-full")}>
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === 'logs' && <ProjectLogsView page={page} />}
              {activeTab === 'plans' && (
