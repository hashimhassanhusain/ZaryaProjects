...
export const IssueLogView: React.FC<IssueLogViewProps> = ({ page }) => {
  ...
  const [editingEntry, setEditingEntry] = useState<IssueEntry | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [isSaving, setIsSaving] = useState(false);
  const [showHistory, setShowHistory] = useState(false);

  const [formData, setFormData] = useState<Partial<IssueEntry>>({
    issueId: '',
    description: '',
    category: 'Technical',
    priority: 'Medium',
  ...
  useEffect(() => {
    ...
    const versionsQuery = query(
      collection(db, 'issue_versions'),
      where('projectId', '==', selectedProject.id),
      orderBy('version', 'desc')
    );

    const unsubEntries = onSnapshot(entriesQuery, (snap) => {
      setEntries(snap.docs.map(d => ({ id: d.id, ...d.data() } as IssueEntry)));
      setLoading(false);
    });
    ...
    const unsubVersions = onSnapshot(versionsQuery, (snap) => {
      setVersions(snap.docs.map(d => ({ id: d.id, ...d.data() } as IssueVersion)));
    });
  ...
  const handleAdd = () => {
    ...
    setFormData({
      issueId: `ISS-${nextNum.toString().padStart(3, '0')}`,
      description: '',
      category: 'Technical',
      priority: 'Medium',
      ownerId: '',
  ...
  const handleEdit = (entry: IssueEntry) => {
  ...
  const handleSave = async (isNewVersion: boolean = false) => {
    ...
    try {
      ...
      if (isNewVersion) {
        ...
        await addDoc(collection(db, 'issue_versions'), {
        ...
        for (const e of entries) {
          await updateDoc(doc(db, 'issues', e.id), {
            version: nextVersion,
            updatedAt: timestamp,
            updatedBy: user
          });
        }
        alert(`Issue Log version v${nextVersion.toFixed(1)} archived.`);
      } else {
        const entryData = {
          ...formData,
          projectId: selectedProject.id,
        ...
        if (editingEntry) {
          await updateDoc(doc(db, 'issues', editingEntry.id), entryData);
        } else {
          await addDoc(collection(db, 'issues'), entryData);
        }
      }

      setView('list');
    } catch (err) {
      handleFirestoreError(err, editingEntry ? OperationType.UPDATE : OperationType.CREATE, 'issues');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const generatePDF = () => {
    ...
    doc.text('PROJECT ISSUE LOG', pageWidth / 2, 35, { align: 'center' });
    ...
    autoTable(doc, {
      startY: 60,
      head: [['ID', 'DESCRIPTION', 'CAT', 'PRIORITY', 'OWNER', 'DATE', 'STATUS', 'RESOLUTION']],
      body: entries.map(e => [
        e.issueId,
        e.description,
        e.category,
    ...
    doc.save(`${selectedProject.code}-ZRY-MGT-LOG-ISS-${dateStr}-V${vStr}.pdf`);
  };
  ...
  const filteredEntries = entries.filter(e => 
    ...
    e.issueId.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-900 tracking-tight">
                  {editingEntry ? 'Edit Issue' : 'Record New Issue'}
                </h2>
                <p className="text-sm text-slate-500 font-medium">Track and resolve project issues as they arise.</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-10 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Issue ID</label>
                <input 
                  type="text"
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Issue Description</label>
              <textarea 
                ...
                placeholder="Describe the issue, its impact, and current situation..."
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Owner</label>
                <select 
              ...
              <div className="space-y-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Date Identified</label>
                <input 
                  type="date"
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Resolution / Action Plan</label>
              <textarea 
                value={formData.resolution}
                onChange={(e) => setFormData({ ...formData, resolution: e.target.value })}
                rows={4}
                className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none resize-none"
                placeholder="Detail the steps taken or planned to resolve the issue..."
              />
            </div>
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-orange-500 rounded-[1.25rem] flex items-center justify-center shadow-xl shadow-orange-200">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest bg-orange-50 px-2 py-0.5 rounded-md">Open Issues: {entries.filter(e => e.status !== 'Closed' && e.status !== 'Resolved').length}</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            ...
            <button 
              ...
              className="px-6 py-2 bg-orange-500 text-white rounded-xl font-bold text-xs hover:bg-orange-600 transition-all"
            >
            ...
            {versions.map((v) => (
              <div key={v.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all">
                <div className="flex items-center gap-6">
                  <div className="text-2xl font-black text-orange-400">v{v.version.toFixed(1)}</div>
                  <div>
                ...
                <tr>
                  <td colSpan={7} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">Loading Issues...</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={7} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">No issues recorded.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredEntries.map((entry) => (
                  ...
                  >
                    <td className="px-8 py-6">
                      <span className="text-xs font-black text-orange-600 bg-orange-50 px-2 py-1 rounded-md">{entry.issueId}</span>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <div className="text-xs text-slate-400 font-mono">{entry.dateIdentified}</div>
                    </td>
                    <td className="px-8 py-6">
                      <span className={cn(
                        "inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest",
                        entry.status === 'Resolved' || entry.status === 'Closed' ? "bg-emerald-100 text-emerald-700" :
