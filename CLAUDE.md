...
1. **PO workPackageId**: Must store `activity.id` (Firestore document ID), NOT `activity.workPackage` (name string). Storing the name string crashes rollupService with "No document to update: activities/Equipment".
...
- To fix root cause: ensure PO is created with `workPackageId: activity.id` (not activity.workPackage name).
...
This is intentional — if dropdown is empty, it means no work package exists for that WBS+Division combination.
Solution: create a work package for that specific WBS level and division first.
