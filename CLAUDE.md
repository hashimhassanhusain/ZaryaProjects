...
## Deployment
- `npm start` runs `tsx server.ts` — this is the production start command.
- Required env vars: `GOOGLE_DRIVE_CREDENTIALS`, `GOOGLE_DRIVE_PARENT_FOLDER_ID`, `GEMINI_API_KEY`.
- See `.env.example` for all required environment variables.
- `tsx` must stay in `dependencies` (not devDependencies) — it is needed at runtime.
